<cfcomponent extends="mura.cfobject" output="false" hint="This provides content rendering utiility methods">
	...
	<cffunction name="addlink" output="false">
		...
		<cfelse><!--- for some reason span itemprop name was not in the line below --->
			<cfset link='<a href="#href#"#iif(len(arguments.target) and arguments.target neq '_self',de(' target="#arguments.target#"'),de(""))##iif(len(theClass),de(' class="#theClass#"'),de(""))##iif(len(arguments.id),de(' id="#arguments.id#"'),de(""))##iif(arguments.showCurrent,de(' #replace(arguments.aCurrentAttributes,"##","####","all")#'),de(""))##iif(arguments.isParent and len(arguments.aHasKidsAttributes),de(' #replace(arguments.aHasKidsAttributes,"##","####","all")#'),de(""))#><span itemprop="name">#HTMLEditFormat(arguments.title)#</span></a>' />
		</cfif>
	...
	<cffunction name="dspCrumblistLinks"  output="false">
		...
		<cfif arrayLen(crumbdata) gt (1 + arguments.renderer.getNavOffSet())>
			<cfsavecontent variable="theNav">
				...
				<!--- asp gotta mess with theOffset to get rid of second "home"	caused by data import--->
                <cfset andysOffset = theOffset -1>
                <cfloop from="#andysOffset#" to="1" index="I" step="-1">
						<cfif I neq 1>
                            <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="<cfif len(arguments.renderer.liBreadcrumbNotCurrentClass)><cfif I eq andysOffset>first </cfif>#arguments.renderer.liBreadcrumbNotCurrentClass#<cfelseif I eq andysOffset>first</cfif>">
								<cfif I neq andysOffset>#arguments.separator#</cfif>
						...
						<cfelse>
							<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="<cfif arraylen(crumbdata)>last<cfelse>first</cfif><cfif len(arguments.renderer.liBreadcrumbNotCurrentClass)> #arguments.renderer.liBreadcrumbCurrentClass#</cfif>">
                                ...
                                <!--- because we copied the data over from an old mura instance, we hosed up the array somehow. I need to subtract one from  arraylen to get the right itemProp. --->
                                <cfset finalPosition = arrayLen(crumbData) - 1>
...
<!--- andy replacing custom code in dspCrumblistLinks with original from GitHub 
	<cffunction name="dspCrumblistLinks"  output="false">
		<cfargument name="id" type="string" default="crumblist">
		<cfargument name="separator" type="string" default="">
		<cfargument name="class" type="string" default="">
		...
		<cfif arrayLen(crumbdata) gt (1 + arguments.renderer.getNavOffSet())>
			<cfsavecontent variable="theNav">
				<cfoutput><ol itemscope itemtype="http://schema.org/BreadcrumbList"<cfif len(arguments.id)> id="#arguments.id#"</cfif> class="mura-breadcrumb<cfif Len(arguments.class)> #arguments.class#</cfif>">
                <!--- Added by andy because of two "Home" links --->
                <!--- 20200429 Had to add  <meta itemprop="position" content="1" />s --->
                <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="first"><a href="/"><span itemprop="name">Home</span></a><meta itemprop="position" content="1" /> &nbsp;&nbsp;/&nbsp;&nbsp;</li>
					<cfloop from="#theOffset#" to="1" index="I" step="-1">
						<cfif I neq 1>
                            <!--- Added by andy because of two "Home" links - had to remove first position --->
                            <cfif crumbdata[I].contentid neq "1">
							<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="<cfif len(arguments.renderer.liBreadcrumbNotCurrentClass)><cfif I eq theOffset>first </cfif>#arguments.renderer.liBreadcrumbNotCurrentClass#<cfelseif I eq theOffset>first</cfif>">
								...
								#arguments.renderer.addlink(
									...
									aCurrentClass=arguments.renderer.aBreadcrumbCurrentClass,
									aNotCurrentClass=arguments.renderer.aBreadcrumbNotCurrentClass)#
									<!--- where should the couner increment <cfset counter=counter+1>--->
									<meta itemprop="position" content="#counter#" />
                                
                                        </li></cfif>
						<cfelse><!--- Added by andy <cfif crumbdata[I].contentid neq "1">--->
							<!---#arrayLen(crumbdata)#---><li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="<cfif arraylen(crumbdata)>last<cfelse>first</cfif><cfif len(arguments.renderer.liBreadcrumbNotCurrentClass)> #arguments.renderer.liBreadcrumbCurrentClass#</cfif>">
								#arguments.separator#
								#arguments.renderer.addlink(type=crumbdata[1].type,
									filename=crumbdata[1].filename,
									title=crumbdata[1].menutitle,
